"""
Train PPO agents for Meta-World push-v3 using 45/5 train-test task splits.

Features
--------
- push-v3 only
- 3 non-overlapping 45/5 task splits by default
- 4 SubprocVecEnv workers by default
- 6M timesteps per model by default
- 4 PPO configs tuned for vectorized training
- optional VecNormalize, ON by default
- checkpoints, TensorBoard logs, split manifests, aggregate CSVs
- final evaluation on train/test task lists

Example

Full run with all configs:
python train_push_v3_splits.py --configs base_push careful_push short_rollout_push light_entropy_push --total-timesteps 6000000 --n-envs 4
"""
...
OUTPUT_DIR = "./pick-place_v3_ppo_split_runs"
...
PPO_CONFIGS: Dict[str, Dict[str, Any]] = {
    ...
    "light_entropy_pick": {
        ...
        "gae_lambda": 0.95,
        "clip_range": 0.20,
        "ent_coef": 0.002,        # light exploration only; high entropy was weak on push before
        "vf_coef": 0.5,
        "max_grad_norm": 0.5,
    },
}
...
def save_and_print_split_manifest(
    ...
    json_path = manifest_dir / f"pick-place_v3_split_{split_id}_seed_{split_seed}_manifest.json"
    with json_path.open("w", encoding="utf-8") as f:
        json.dump(rows, f, indent=2)

    # CSV keeps only compact columns because decoded_data can be nested.
    csv_path = manifest_dir / f"pick-place_v3_split_{split_id}_seed_{split_seed}_manifest.csv"
    compact_rows = []
    for r in rows:
        compact_rows.append(
            {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train pick-place-v3 PPO models with 45/5 splits and SubprocVecEnv.")
    ...
    aggregate_csv = result_dir / "pick-place_v3_aggregate_results.csv"
    per_episode_csv = result_dir / "pick-place_v3_per_episode_results.csv"
    skipped_csv = result_dir / "pICK-PLACE_v3_skipped_runs.csv"

    print("=" * 110)
    print("PPICK-PLACE-V3 PPO TRAINING")
    print("=" * 110)
    ...
    print("\nExtracting 50 pick-place-v3 task variations ...")
